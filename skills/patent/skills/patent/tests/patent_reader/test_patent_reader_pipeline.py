...
def main() -> int:
    ...
    r = run(
        [
            sys.executable,
            str(ROOT / "tools" / "patent_reader" / "extract" / "extract_patent_text.py"),
            "-i",
            str(SAMPLE),
            "-o",
            str(out),
    ...
    r2 = run(
        [sys.executable, str(ROOT / "tools" / "patent_reader" / "analyze" / "build_context_anchor.py"), "-w", str(out)]
    )
    if r2.returncode != 0:
    ...
    r2b = run(
        [
            sys.executable,
            str(ROOT / "tools" / "patent_reader" / "analyze" / "build_claim_mermaid.py"),
            "--claim-tree",
    ...
    r3 = run(
        [
            sys.executable,
            str(ROOT / "tools" / "patent_reader" / "analyze" / "lint_patent_note.py"),
            "--note",
            str(note),
            "--manifest",
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        r_setup = run(
            [sys.executable, str(ROOT / "tools" / "patent_reader" / "vault" / "setup_obsidian_vault.py")],
            env=env,
        )
        ...
        r4 = run(
            [
                sys.executable,
                str(ROOT / "tools" / "patent_reader" / "vault" / "write_patent_obsidian_note.py"),
                "--content-file",
                str(note),
                "--manifest",
                str(out / "source_manifest.json"),
        ...
        r_clues = run(
            [
                sys.executable,
                str(ROOT / "tools" / "patent_reader" / "analyze" / "validate_public_clues.py"),
                "-i",
                str(out / "public_clues.json"),
            ]
        )

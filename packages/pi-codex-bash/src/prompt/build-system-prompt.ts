...
const NORMAL_CODEX_GUIDELINES = [
	"Use exec_command for shell commands, file inspection, builds, tests, and file deletion; prefer rg / rg --files for discovery and focused commands over truncation",
	"Reserve tty=true for input or persistent processes",
	"Use the edit tool for targeted changes to existing files with exact oldText/newText matches; use the write tool to create files or rewrite them entirely",
...
function buildCodexGuidelines(piPackageRoot?: string): string[] {
	const guidelines = [...NORMAL_CODEX_GUIDELINES];
	if (piPackageRoot) {
...
function injectGuidelines(prompt: string): string {
	...
	if (!match || match.index === undefined) {
		const fallbackSection = `Guidelines:\n${buildCodexGuidelines().map((line) => `- ${line}`).join("\n")}`;
	...
	const keptBodyLines = canonicalBodyLines;
	const existingLines = keptBodyLines
		.map((line) => line.trim())
		.filter((line) => line.startsWith("- "));
	const existing = new Set(existingLines.map((line) => line.slice(2)));
	const additions = buildCodexGuidelines().filter((line) => !existing.has(line)).map((line) => `- ${line}`);
...
function buildHeavyCodexSystemPrompt(
	...
	const piPackageRoot = !source.customPrompt && stripPiDocumentation(prompt) !== prompt
		...
		: undefined;
	prompt = replaceHeavyGuidelines(prompt, buildCodexGuidelines(piPackageRoot));
